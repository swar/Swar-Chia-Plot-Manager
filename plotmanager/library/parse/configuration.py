...
def _get_manager_settings(config):
    ...
    expected_parameters = ['manager_name', 'check_interval', 'log_level']
    _check_parameters(parameter=manager, expected_parameters=expected_parameters, parameter_type='manager')
    return manager['manager_name'], manager['check_interval'], manager['log_level']
...
def _get_global_config(config):
    ...
    expected_parameters = ['max_concurrent', 'destination_directory', 'max_for_phase_1', 'minimum_minutes_between_jobs']
    ...
    destination_directory = global_config['destination_directory']
    ...
    if not isinstance(destination_directory, list):
        raise Exception('global -> destination_directory should be list value.')
    ...
    if not isinstance(minimum_minutes_between_jobs, int):
        raise Exception('global -> max_concurrent should be a integer value.')
    return max_concurrent, destination_directory, max_for_phase_1, minimum_minutes_between_jobs
...
def get_config_info():
    ...
    manager_name, manager_check_interval, log_level = _get_manager_settings(config=config)
    log_directory = _get_log_settings(config=config)
    if not os.path.exists(log_directory):
        os.makedirs(log_directory)
    jobs = _get_jobs(config=config)
    max_concurrent, destination_directory, max_for_phase_1, minimum_minutes_between_jobs = _get_global_config(config=config)
    ...
    return chia_location, log_directory, jobs, manager_name, manager_check_interval, max_concurrent, max_for_phase_1, \
        destination_directory, minimum_minutes_between_jobs, progress_settings, notification_settings, log_level, \
