...
def _get_row_info(pid, running_work, num):
    ...
    row = [num, work.job.name if work.job else '?', pid, work.datetime_start.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    for job in jobs:
        for pid in job.running_work:
            if pid not in running_work:
                continue
            statuses.append(_get_row_info(pid, running_work, num))
            num += 1
