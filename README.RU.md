...
* Распределение засев полей так, чтобы избежать высоких накрузок на ресурсы вашего компьютера.
* Доступен список каталогов назначения.
* Использует потенциал временного хранилища по максимуму, используя предварительный старт нового участка.
* Запускает максимальное количество участков одновременно, избегая конкуренции полей и ограничений использования ресурсов.
...
* Некоторым пользователям нравится указывать одининаковый каталог для temporary2 и destination. При включении этого параметра в качестве диска назначения всегда будет использоваться диск temporary2. Используя этот параметр, вы можете указать для temporary2 пустой список.
...
Эта команда запустит менеджер в фоновом режиме. После того, как вы запустите его, он всегда будет работать, пока задания не завершили свои `max_plots` или не возникнет ошибка. Ошибки будут регистрироваться в созданном файле `debug.log`.

### stop

Эта команда завершит работу менеджера в фоновом режиме. Это не останавливает работающие задачи создания полей, лишь прекращает создание новых.

### restart

Это команда последовательно выполнит start и stop.

### view

Эта команда покажет вид, который вы можете использовать для отслеживания засева полей. Он будет обновляться каждые X секунд, определяемых вашим `config.yaml`.

### status

Эта команда отобразит текущее состояние вида аналогичного view, только без периодического обновления.

### analyze_logs

Эта команда проанализирует все завершенные журналы создания плотов в вашей папке журналов и вычислит правильные веса и концы фаз для вашего компьютера. Просто введите возвращенные значения в раздел `progress` вашего файла `config.yaml`. Это влияет только на индикатор выполнения.


## Установка [Нужен перевод]

#### ВНИМАНИЕ: Если `python` не работает, попробуйте `python3`.
...
5. Установите необходимые модули: `pip install -r requirements.txt`
	* Если Вы планируетее использовать оповещения или Prometheus установите так же дополнительные модули: `pip install -r requirements-notification.txt`
...
10. Запустите режим просмотра: `python manager.py view`
   * Это приведет к циклическому просмотру экрана с подробными сведениями об активных процессах засева полей.
...
Настройки, позволяющие подключить сбор метрик через Prometheus.

* `prometheus_enabled` - Если этот параметр включен, будут собираться метрики и запускаться HTTP-сервер, чтобы предоставлять их для Prometheus.
* `prometheus_port` - Порт HTTP сервера.

Список собираемых метрик

- **chia_running_plots**: A [Gauge](https://prometheus.io/docs/concepts/metric_types/#gauge) чтобы узнать, сколько полей сейчас создается .
- **chia_completed_plots**: A [Counter](https://prometheus.io/docs/concepts/metric_types/#counter) для завершенных полей.
